...
def fc_layer(_input, size, name, activation_function='leaky_relu'):
    """
...
def mlp_fn(features,  input_size, n_classes, activation_function):
    ...
    with tf.variable_scope("mlp_scope"):
        ...
        fc1 = fc_layer(features, 100, name = 'fc1', activation_function = activation_function)
        #fc5 = layers.dropout_layer(fc5, 0.8)
        print(" fc1: {} ".format(fc1.get_shape().as_list()))
        #fc 6
        fc2 = fc_layer(fc1, 100, name = 'fc2', activation_function = activation_function)
        print(" fc2: {} ".format(fc2.get_shape().as_list()))
        #fully connected
        fc3 = fc_layer(fc2, n_classes, name = 'fc3', activation_function='None')
...
def model_fn (features, labels, mode, params):
    ...
    net = mlp_fn(features, params['input_size'], params['number_of_classes'], params['activation_function'])
    #    
    output = net["output"]
    ...
    if mode == tf.estimator.ModeKeys.PREDICT:
        ...
        elif 'Gradient' in params['optimizer']:
